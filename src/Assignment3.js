...
function WriteUp(){
  return(
    <div>
      ...
      <p>We split the teamwork into three different roles: 1) team members who created the graphics (Chloe and Phung), 2) a team member who routed the project to React (Annie), and 3) team members who created the write-up (Amy and Angela). The team also met to discuss roles, connect over Messenger, and decide on which dataset to use. The repository that we decided to build off of was initially developed by Chloe. Chloe and Phung spent over 2 hours creating the interactive visualization. Annie spent around 1-2 hours setting up React Router. Amy and Angela spent about an hour writing, editing, and finalizing the write-up. The aspect that took the most time was adding interactivity to the static visualizations because we wanted to implement multiple interactive features, so getting all of the features to work together on the visualization was a challenge. </p>
      //<br />
    </div>
  );
}
